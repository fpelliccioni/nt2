...
namespace boost { namespace simd { namespace ext
{
                                    ...
                                    )
  {
    ...
    {
      BOOST_ASSERT_MSG
      ( boost::simd::is_aligned(a0+a1-A3::value,sizeof(result_type))
      , "load has been called on a pointer which alignment is not "
        "compatible with current SIMD extension."
      );

      return eval( a0, a1, boost::mpl::bool_<!unalignment>() );
    }

    // Periodic case - Just load as normal
    BOOST_FORCEINLINE result_type
    eval( A0 const& a0, A1 a1, boost::mpl::true_ const&) const
    {
      return boost::simd::load<result_type>(a0,a1);
    }

    // Forward, non-periodic case
    BOOST_FORCEINLINE result_type
    eval(A0 const& a0, A1 a1, boost::mpl::false_ const&) const
    {
